...
function parseOBJ(text) {
  ...
  for (var g of geometries) {
    ...
    for (var i = 0; i < g.data.position.length; i++) {
      newPos.push(
        (g.data.position[i] - vertexMinVal) / (vertexMaxVal - vertexMinVal)
      );
    }
...
function parseMTL(text) {
  ...
  const keywords = {
    ...
    map_Ka(parts, texturePath) {
      material.ambientMap = initTextures(texturePath);
    },
    map_Kd(parts, texturePath) {
      material.diffuseMap = initTextures(texturePath);
    },
    map_Ns(parts, texturePath) {
      material.specularMap = initTextures(texturePath);
    },
    map_Bump(parts, texturePath) {
      material.normalMap = initTextures(texturePath);
    },
    Ni(parts) {
...
function initTextures(texturePath) {
  ...
  currTexture.image.onload = function () {
    handleTextureLoaded(currTexture);
  };
